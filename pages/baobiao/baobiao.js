...
Page({
  ...
  allx: function () {
    ...
    wx.request({
      url: AjaxUrl.AjaxUrl + 'getSellStaticCGUnitTotal.ashx',
      data: {
        openID: opends,
        CGUnitName: that.data.inputs,
        startDate: that.data.date1,
  ...
  totameterfunction: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data.head.status == 'success') {
          that.setData({
            totameter: res.data.body[0].totalMeter,
          })
        } else {
          wx.showToast({
            title: res.data.head.msg,
  ...
  busectionclick: function (e) {
    var that = this
    // console.log(e)
    // console.log(that.data.busection)
    // console.log(that.data.idsval)
    that.setData({
  ...
  ajaallday: function () {
    var that = this
    // console.log(that.data.ajaallday2+'------开始日期')
    // console.log(that.data.ajaallday1 + '------结束日期')
    wx.request({
      url: AjaxUrl.AjaxUrl + 'getSellStatic.ashx',
      data: {
        openID: opends,
