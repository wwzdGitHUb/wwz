...
Page({
  ...
  shezhi: function (e) {
    ...
    if (e.currentTarget.dataset.je == undefined || e.currentTarget.dataset.prices == undefined) {
      e.currentTarget.dataset.je = '',
        e.currentTarget.dataset.prices = ''
    }
    that.setData({
      hiddemsg: true,
      ...
      bzgoods: e.currentTarget.dataset.bj,


      allmonesa: e.currentTarget.dataset.je,
      pricenum: e.currentTarget.dataset.prices,
    })
    ...
    for (var i = 0; i < dest.length; i++) {
      ...
      if (dest[i].data.length > 1) {
        for (var j = 0; j < dest[i].data.length; j++) {
          ...
          // dest[i].data[0].meterList.str.split(',');
          // console.log(dest[j].data[0].meterList)
          // countPs.push(dest[i].data[0].meterList.split(','))
          // console.log(countPs)
          // dest[i].data[0].countP = countPs[j]
  ...
  allpricemoney: function () {
    ...
    for (var i = 0; i < alls.length; i++) {
      for (var j = 0; j < alls[i].meterList.split(',').length;j++){
        p.push(alls[i].meterList[j].split(','))
      }
